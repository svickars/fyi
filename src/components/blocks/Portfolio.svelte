<script>
  ...
  import markup from "../../../public/assets/copy/markup.json";
  import NotWork from "./NotWork.svelte";
  import {
    allClosed,
    allOpen,
    somethingOpen,
  ...
  function handleExpand() {
    ...
    if ($allOpen || $somethingOpen) {
      ...
      projectChildren.forEach(
        (project, i) =>
          (openTimeout[i] = setTimeout(() => project.openProject(), i * 250))
      );
      $allOpen = true;
      $allClosed = false;
    }
...
<div class="w-full animate-pop-up-fast mb-60">
  <div
    class="flex flex-col w-full max-w-5xl px-8 mx-auto mb-4 md:flex-row md:justify-between"
  >
