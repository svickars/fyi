...
>
    ...
    <div class="relative">
      ...
      >
        <div class="flex flex-row items-center order-2 md:order-1">
          <i
            class="mr-3 fa-thin fa-xl fa-arrow-right transition cursor-pointer {hover &&
            !open
  ...
  {#if open}
    <div transition:slide>
      ...
      {#if project.media}
        <div
          class="grid w-full gap-4 px-4 mx-auto my-16 cols-4 md:grid-cols-12 max-w-7xl"
        >
      ...
      <div class="w-full max-w-5xl px-8 pb-6 mx-auto">
        <div style="padding-left: {arrowW + 12}px">
          {#each project.description as text}
            <p
              class="max-w-3xl mb-4 font-sans text-base leading-relaxed md:text-lg lg:text-xl"
            >
              {@html text.value}
            </p>
          {/each}
          {#if project.links}
            <Launch
              colour={project.dark ? "white" : project.color}
